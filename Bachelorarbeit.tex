...
%				\todo[inline]{more examples \cite[Definition 2.2]{srinavsan-thesis}}
	...
	\begin{proof}
		...
		\begin{center}
			...
			\end{tikzcd}
		\end{center}
		The right strand of the above diagram is, again, the identity by definition of the dual object. 
			...
			\begin{sideways}
				\begin{tikzcd}[every label/.append style = {font = \tiny}, 
					cells={nodes={font=\small}}]
						A \otimes B
							\arrow[d, equal ]
							\arrow[r, "\id \otimes (\id \otimes \tau_B)\rho_\otimes^{-1} " ]
						&
						...
						A \otimes ( B \otimes ( B^* \parr ((A^* \parr A) \otimes B) ) )
							...
							\arrow[ddl, "\id \otimes \partial_L "' ]
					\\
						A\otimes B
							\arrow[d, "\id \otimes ( \tau_A \otimes \id ) \lambda_\otimes^{-1} "' ]
						& 
						A \otimes ( (B \otimes B^*) \parr B )
							\arrow[l, "\id \otimes \lambda_\parr (\gamma_B \parr \id) " ]
					...
					\\
						...
						&
						A \otimes ( (B \otimes B^*) \parr ((A^* \parr A) \otimes B) )
							\arrow[l, "\id \otimes \lambda_\parr (\gamma_B \parr \id) " yshift=-5pt ]
						&
				\end{tikzcd}
			\end{sideways}
		\end{center}
	...
	\begin{proof}
		...
		\begin{center}
			...
			\begin{tikzcd}
					&
					B^*
						\arrow[d, "(\tau^*_A \otimes \id) \lambda_\otimes^{-1} " ]
					&
				\\
					&
					(A^* \parr A) \otimes B^*
		...
		\begin{center}
			...
			\begin{tikzcd}
					&
					{^*B}
					\arrow[d, "({^*}\tau_A \otimes \id) \rho_\otimes^{-1} " ]
					&
				\\
					&
					{^*B} \otimes (A \parr {^*A})
				...
				\\
					&
					A^* \parr (B \otimes B^*)
					\arrow[d, "\lambda_\parr (\id \parr {^*\gamma_B}) " ]
					&
				\\
					&
					A^*
		...
		\begin{center}
			\begin{tikzcd}[sep=small]
					...
					(A^* \parr A) \otimes C^*
						...
						\arrow[dr, "\id \otimes (\tau^*_A \otimes \id) \lambda^{-1} "' ]
					&
					&
					(A^* \parr B) \otimes C^*
					\arrow[dr, "(\id \parr \gamma^*_C) \partial_R ((\id \parr g) \otimes \id) " ]
					&
				\\
					C^*
						\arrow[ur, "(\tau^*_A \otimes \id) \lambda^{-1} " ]
						\arrow[dr, "(\tau^*_B \otimes \id) \lambda^{-1} "' ]
					&
					&
					(A^* \parr A) \otimes (B^* \parr B) \otimes C^*
						\arrow[ur, "\cdots\gamma^*_B\cdots \circ \cdots\partial_R\partial_L\cdots \circ \cdots f \cdots " {shift right}]
						\arrow[dr, "(\id \parr \gamma^*_C) \partial_R ((\id \parr g) \otimes \id) "']
					&
					&
					A^*
				\\
					...
					&
					(A^* \parr A) \otimes B^*
						\arrow[ur, "(\id \parr \gamma^*_B) \partial_R ((\id \parr f) \otimes \id) "' ]
					&
				\\
					&
					&
		...
		\begin{center}
			...
			\begin{tikzcd}
					{^*(A^*)}
						\arrow[r, "\sim "]
						\arrow[d, "(\id \otimes {^*\tau}_{B^*}) \lambda_\otimes^{-1} "' ]
					&
					A
						\arrow[d, "(\id \otimes \tau^*_B) \lambda_\otimes^{-1} " ]
				\\
					{^*(A^*)} \otimes (B^* \parr {^*(B^*))}
						\arrow[r, "\sim "]
				...
				\\
					({^*(A^*)} \otimes A^*) \parr {^*(B^*)}
						\arrow[r, "\sim "]
						\arrow[d, "\rho_\parr ({^*\gamma}_{A^*} \parr \id) "' ]
					&
					(A \otimes A^*) \parr B
						\arrow[d, "\rho_\parr (\gamma^*_{A} \parr \id) " ]
				\\
		...
		\begin{center}
			\begin{tikzcd}
					A
						\arrow[r, "f"]
						\arrow[d, "(\id \otimes \tau^*_B) \rho_\otimes^{-1} "' ]
					&
					B
						\arrow[d, "(\id \otimes \tau^*_B) \rho_\otimes^{-1} " ]
				\\
		...
		\todo[inline]{big diagram for last square?}
...
%			
	...
	\begin{remark}
		...
		On the other hand, when starting with intuitionistic multiplicative linear logic (cf. Melliès for a list of its calculus rules \cite[p43, fig. 2]{mellies}), which forms a (bi-)closed monoidal category, we obtain a *-autonomous category as defined by Barr \cite{barr95}\cite{barr91} when we adopt a classical negation, thus giving additional motivation to theorem \ref{equivalenceAutoCats}.
