...
\begin{document}
	...
	Furthermore, note that the contexts in the unit introduction rules are arbitrary, and that each unit only has one introduction rule as opposed to the multiplicative units.
	...
	\begin{definition}
		We call two formulas $A$ and $B $ \emph{(linearly) equivalent} iff $A \vdash B$ and $B \vdash A $ are provable.
	\end{definition}
	...
	\begin{center}
		...
		\end{align*}
	\end{center}
	\todo[inline]{define left hand side rules! (or declare above equations as definitions... but more rules is more good)}
