...
%		comment.....................................................................................................
	...
	\begin{lemma}
		...
		\cite[Prop. 4.4.1]{riehl_context}
		If $F\dashv G$ and $F' \dashv G$, then $F$ and $F'$ as well as the (co-)units are uniquely isomorphic. The same follows for $G$ and $G'$ if $F \dashv G$ and $F \dashv G'$.
	\end{lemma}
	
	\begin{lemma}
		\label{lemma: compAdjunction}
		 \cite[Prop. 4.4.4]{riehl_context} 
		Adjunctions compose: Given $F: \category{C} \rightarrow \category{D}$ and $F': \category{D} \rightarrow \category{E}$ with $F\dashv G$ and $F' \dashv G'$, then $F'F \dashv GG'$.
		The unit and co-unit are given by:
		\begin{align*}
			\tilde{\eta}: & \id_{\category{C}} \overset{\eta}{\longrightarrow} GF \overset{G \eta' F}{\longrightarrow} GG' F'F
			\\
			\tilde{\varepsilon}: & FF' G'G \overset{F '\varepsilon' F}{\longrightarrow} FG \overset{\varepsilon}{\longrightarrow} \id_{\category{E}}
		\end{align*}
		with $\eta F := \eta_{F(-)} $ etc.
	\end{lemma}
	...
	\begin{definition}[Symmetric monoidal category]
		...
		\begin{itemize}
			...
			\item 
				the unit law: $\lambda = \rho \sigma$
				\begin{center}
					\begin{tikzcd}
						\Unit \otimes A 
						\arrow[rr, "\sigma_{\Unit, A} "]
						\arrow[rd, "\lambda "' ]
						&
						&
						A \otimes \Unit
...
%				\todo[inline]{more examples \cite[Definition 2.2]{srinavsan-thesis}}
		...
		\begin{center}
			\begin{sideways}
				\begin{tikzcd}[every label/.append style = {font = \tiny}, 
					cells={nodes={font=\small}}, row sep=large]
						A \otimes B
							\arrow[d, equal ]
	...
	\begin{proof}
		...
		As the squares commute, the order of distributor and $f$ does not matter. For duality to be a functor, it needs to preserve composition, leading to the following diagram in the case of $(-)^*$ (suppressing $\alpha_\otimes$ and some identities for better readability):
		...
		The path at the top of the diagram is $(gf)^*$ while the path at the bottom is $f^*g^*$. Commutativity of the diagram is shown in the same way as in Lemma \ref{lemma: isoLinDisCat}.
			...
			\begin{sideways}
				\begin{tikzcd}[row sep=large ]
					...
					\\
						...
						&
						( (A \otimes A^*) \parr (A \otimes B^*) ) \parr B
							\arrow[r, " (\gamma_A \parr \id) \otimes \id " {yshift=4pt} ]
					...
					\\
						...
						&
						( (A \otimes A^*) \parr (B \otimes B^*) ) \parr B
							\arrow[r, "(\gamma_A \parr \id) \parr \id " {yshift=4pt} ]
		...
		Similarly, we obtain 		
		\begin{align*}
			{^*}(B^* \otimes A^*) \cong ({^*B} \otimes {^*A})^* = A\parr B 
		\end{align*}
		...
		\begin{center}
			\begin{tikzcd}[sep=large]
					...
					&
					G' F' F G'
						\arrow[r, "G' \alpha^{-1}_\otimes G' "]
						\arrow[d, "G'F'FG' \eta^L " ]
					&
					G' F F' G'
						\arrow[r, "G' F \epsilon^R " ]
						\arrow[d, "G'F F' G' \eta^L " ]
					&
					G' F
						\arrow[d, "G'F \eta^L " ]
				\\
					F G' G F
						\arrow[r, "\eta^R FG' GF "]
						\arrow[d, "F \alpha_\parr^{-1} F"]
					&
					G'F' FG' GF
						\arrow[r, "G' \alpha^{-1}_\otimes G' GF "]
						\arrow[d, "G'F' F \alpha_\parr^{-1} F"]
					&
					G'F F'G' GF
						\arrow[r, "G' F \epsilon^R " ]
					&
					G'F GF
						\arrow[dd, "G' \epsilon^L F "]
				\\
					FG G'F
						\arrow[r, "\eta^R FG G'F "]
						\arrow[d, "\epsilon^L G'F "]
					&
					G'F' FG G'F
						\arrow[d, "G'F' \epsilon^L G'F "]
					&&
				\\
					G'F
						\arrow[r, "\eta^R G'F "]
					&
					G'F' G'F
						\arrow[rr, "G' \epsilon^R F "]
					&&
					G'F
			\end{tikzcd}
		\end{center}
		
		The paths on the top and left are the two definitions of $\partial_L$, the paths on the right and bottom are the identity on $G'F$ by the triangle identity of Proposition \ref{prop.: triangleIdentity}. The small squares commute by naturality of the (co-)units, the big square does so by Lemmas \ref{lemma: isoAdjunctions} and \ref{lemma: compAdjunction}.
		Switching $F$ for $F'$, $G'$ for $G$, and the appropriate morphisms gives us the same diagram for $\partial_R$.
		
		Thus we can show the coherence conditions by transposing the diagram along either adjunction:
		...
		follows from the commutativity of its transpose under ${^*A} \otimes - \dashv A \parr - $:
		\begin{center}
			\begin{tikzcd}
				{^*A} \otimes ((A \parr B) \otimes 1)
					\arrow[r, "\alpha_\otimes " ]
					\arrow[d, " \id \otimes \rho_\otimes " ]
				&
				({^*A} \otimes (A \parr B)) \otimes 1
					\arrow[r, "\epsilon^L \otimes \id " ]
					\arrow[d, "\rho_\otimes " ]
				&
				B \otimes 1
					\arrow[d, "\rho_\otimes " ]
		...
		Similarly, coherence of $\partial_R$ and $\lambda_\parr$
		\begin{center}
			\begin{tikzcd}
				(\bot \parr A) \otimes B
					\arrow[r, "\partial_R " ]
					\arrow[d, "\lambda_\parr \otimes \id " ]
				&
				\bot \parr (A \otimes B)
					\arrow[d, "\lambda_\parr " ]
				\\
		...
		follows from commutativity of its transpose under $- \otimes B \dashv - \parr {^*B} $:
		\begin{center}
			\begin{tikzcd}
				\bot \parr A
					\arrow[r, "\id \parr \eta^R " ]
					\arrow[d, "\lambda_\parr " ]
				&
				\bot \parr ((A \otimes B) \parr {^*B})
					\arrow[r, "\alpha_\parr^{-1} " ]
					\arrow[d, "\lambda_\parr " ]
				&
				(\bot \parr (A \otimes B)) \parr {^*B}
					\arrow[d, "\lambda_\parr \parr \id " ]
				\\
				A
					\arrow[r, "\eta^R " ]
				&
		...
		Coherence with the associator $\alpha_\otimes$
		\begin{center}
			\begin{tikzcd}
				(A \parr B) \otimes (C \otimes D)
					\arrow[r, "\partial_R " ]
					\arrow[d, "\alpha_\otimes " ]
				&
				A \par (B \otimes (C \otimes D))
					\arrow[dd, "\id \parr \alpha_\parr " ]
				\\
				((A \parr B) \otimes C) \otimes D
					\arrow[d, "\partial_R \otimes \id " ]
				&
		...
		also follows from its transpose:
		\begin{center}
			\begin{tikzcd}
					{^*A} \otimes ((A \parr B) \otimes (C \otimes D))
						\arrow[r, "\alpha_\otimes " ]
						\arrow[dd, "\id \otimes \alpha_\otimes " ]
					&
					({^*A} \otimes (A \parr B)) \otimes (C \otimes D)
						\arrow[r, "\epsilon^L \otimes \id " ]
						\arrow[d, "\alpha_\otimes " ]
				...
				\\
					&
					(({^*A} \otimes (A \parr B)) \otimes C) \otimes D
						\arrow[r, "(\epsilon^L \otimes \id) \otimes \id " ]
					&
					(B \otimes C) \otimes D
						\arrow[dd, equal]
				\\
					{^*A} \otimes (((A \parr B) \otimes C) \otimes D)
						\arrow[r, "\alpha " ]
						\arrow[d, "\id \otimes (\partial_R \otimes \id) " ]
					&
					( {^*A} \otimes ( (A \parr B) \otimes C ) ) \otimes D
						\arrow[d, " (\id \otimes \partial_R) \otimes \id " ]
						\arrow[u, "\alpha \otimes \id " ]
				\\
					{^*A} \otimes ((A \parr (B \otimes C)) \otimes D)
						\arrow[r, "\alpha " ]
					&
		...
		The bottom-right square is the transposed of the following diagram (before application of the functor $-\otimes D$):
		\begin{center}
			\begin{tikzcd}
				(A \parr B) \otimes C
					\arrow[r, "\partial_R " ]
				...
				&
				A \parr (B \otimes C)
					\arrow[d, equal, "\id_A \parr \id_{B \otimes C} " ]
				\\
				A \parr (B \otimes C)
					\arrow[r, equal, "\id_{A \parr (B \otimes C)} " ]
				&
				A \parr (B \otimes C)
			\end{tikzcd}
		\end{center}
		
		The remaining coherence conditions between the distributors and the associators or unitors are shown in the same way.
		
		Coherence between $\partial_L$ and $\partial_R$ requires more work as we have to deal with two adjunctions at once. 
		We notate $F:= A \otimes - \dashv A^* \parr - =: G$ and $F':= -\otimes D \dashv -\parr {^*D}$ as well as $FF' \cong F'F$ and $G'G \cong GG'$, again.
		First, note that the transpose of
		\begin{equation*}
			FF'a \overset{Ff}{\longrightarrow} Fb \overset{h}{\longrightarrow} d
		\end{equation*}
		
		along the adjunction $(\tilde{\eta}, \tilde{\varepsilon}): FF' \dashv G'G $ is given by
		\begin{center}
			\begin{tikzcd}
					&
					a
		...
		Here, $\check{f}$ denotes the transpose of $f$ along the adjunction $F' \dashv G'$, while $\hat{h}$ is the transpose of $h$ along $F \dashv G$.
		On the other hand, the transpose of 
		\begin{equation*}
			F'Fa \overset{F'g}{\longrightarrow} F'c \overset{k}{\longrightarrow} d
		\end{equation*}
		
		along the adjunction $(\bar{\eta}, \bar{\varepsilon}): F'F \dashv GG'$ is given by
		\begin{center}
			\begin{tikzcd}
					a
						\arrow[d, "\eta" ]
						\arrow[dr, "\bar{\eta} " ]
						\arrow[dd, bend right=50 , "\hat{g} "' ]
					&
				\\
					GF a
						\arrow[r, " G \eta' F" ]
			...
			\end{tikzcd}
		\end{center}
		Again, $\check{k}$ denotes the transpose of $k$ along the adjunction $F' \dashv G'$, while $\hat{g}$ is the transpose of $g$ along $F \dashv G$.
		
		Furthermore, the following diagram commutes, with the right-hand path being the transpose along $FF'\dashv G'G$:
		\begin{center}
			\begin{tikzcd}
					a
						\arrow[d, "\bar{\eta} " ]
						\arrow[dr, "\tilde{\alpha}_\otimes = \alpha_\otimes \circ \tilde{\eta} " ]
					&
				\\
					GG' F'F a
						\arrow[r, "\sim ", "\alpha_\parr "' ]
						\arrow[d, "GG' F' g " ]
					&
					G'G F'F a
						\arrow[d, "G'G F' g " ]
		...
		Thus, when checking whether the following diagram commutes
		\begin{center}
			\begin{tikzcd}
					FF' a
						\arrow[r, "Ff"] 
						\arrow[d, "\sim" {above, rotate=90, xshift=2pt, yshift=-1pt}]
					&
					F b
						\arrow[dd, "h"]
				\\
		...
		it is sufficient to check the following diagram:
		\begin{center}
			\begin{tikzcd}
					&
					a
		...
		Translating this back into our original situation gives us the following commuting diagram (recall the transposes of $\partial_L$ and $\partial_R$ under $A \otimes - \dashv A^* \parr - $ and $ -\otimes D \dashv -\parr {^*D} $ resp.):
		\begin{center}
			\begin{sideways}
				\begin{tikzcd}[row sep=large]
						&
						B \parr C
							\arrow[dl, " \eta^L \parr \id "' ]
							\arrow[dr, " \id \parr \eta^R " ]
						&
					\\
						(A^* \parr (A \otimes B)) \parr C
							\arrow[r, "\id \parr \eta^R "' ]
							\arrow[d, "\alpha_\parr^{-1} "' ]
						&
						(A^* \parr (A \otimes B)) \parr ((C \otimes D) \parr {^*D})
							\arrow[ddl, bend left=10, "\alpha_\parr^{-1} " ]
							\arrow[ddr, bend right=10, "\alpha_\parr "' ]
						&
						B \parr ((C \otimes D) \parr {^*D})
							\arrow[l,  " \eta^L \parr \id " ]
							\arrow[d, "\alpha_\parr" ]
					\\
						A^* \parr ((A \otimes B) \parr C)
							\arrow[d, "\id \parr (\id \parr \eta^L) "' ]
						&&
						( B \parr (C \otimes D) ) \parr {^*D}
							\arrow[d, " (\eta^L \parr \id) \parr \id " ]
					\\
						A^* \parr ( (A \otimes B) \parr ( (C \otimes D) \parr {^*D} ) )
							\arrow[d, "\id \parr \alpha_\parr "' ]
						&&
						( ( A^* \parr (A \otimes B) ) \parr (C \otimes D) ) \parr {^*D}
							\arrow[d, "\alpha_\parr^{-1} \parr \id " ]
					\\
						A^* \parr ( ((A \otimes B) \parr (C \otimes D)) \parr {^*D} )
							\arrow[rr, "\alpha_\parr " ]
						&&
						(  A^* \parr ((A \otimes B)  \parr (C \otimes D)) ) \parr {^*D}
				\end{tikzcd}
			\end{sideways}
		...
		Thus, we obtain the following coherence diagram:
		\begin{center}
			\begin{tikzcd}
				&
				A \otimes ((B \parr C) \otimes D)
		...
		The other $\partial_L$-$\partial_R$-coherence is shown in the same way.
		...
		Well-definedness and the defining diagrams are shown by above methods.
	\end{proof}
...
%	\todo[inline]{main source: IntroLinLog}
	...
	\begin{definition}[Formula]
		...
		\end{itemize}
		Note that the connective $(-)^\bot$ is the linear negation and not to be confused with the constant $\bot$, the multiplicative falsehood. 
	...
	\begin{remark}
		While the turnstile serves a purely syntactic purpose, we will later see that it is correctly interpreted as an implication.
		$1$ and $\top$ are the multiplicative and additive truths, $\bot$ and $0$ the resp. falsities.
	\end{remark}
	...
	As already mentioned, the classical conjunction $\wedge$ and disjunction $\vee$ as well as their respective units split into two respectively. 
	These can be classified as multiplicative and additive connectives (analogous to the classically equivalent ways to define $\wedge$ and $\vee$). 
	Using the exponential connectives, one can then encode intuitionistic and classical logic inside of linear logic, again \cite[Chapter 5]{girard87}.
	...
	\begin{theorem}
		...
		\begin{itemize}
			...
			\begin{align*}
				\left(A \otimes B\right)^\bot 
				&\equiv A^\bot \parr B^\bot 
				&
				\left(A \parr B\right)^\bot 
				&\equiv A^\bot \otimes B^\bot 
				\\
				\left(A \with B\right)^\bot 
				&\equiv A^\bot \oplus B^\bot 
				&
				\left(A \oplus B\right)^\bot 
				&\equiv A^\bot \with B^\bot 
			\end{align*}
		\end{itemize}
	\end{theorem}
...
%	\formatnote{proofsymbol なんで？ :( }
	...
	\begin{proof}
		...
		\begin{itemize}
			\item If one of the subproofs is an axiom, we just take the remaining one, resulting in a proof with a lower degree:
			...
			\item Finally, if both $\tau_1$ and $\tau_2$ introduce the principle formula with their last rule, we perform one of the following transformations resulting in a proof of lower degree:
...
%comment.................................................................................................................................
	...
	\begin{definition}[Cotensoriality]
		A proof invariant is called \emph{cotensorial} iff there is a binary operation $\parr$ such that for any two proofs
		\AxiomC{$\pi_1 $}
		\noLine
		\UnaryInfC{$\svdots$}
		\UnaryInfC{$A \vdash C$} 
...
%comment................................................................................................................................................
	...
	\begin{lemma}
		Any (co-)tensorial, modular proof invariant forms a symmetric monoidal category with (denotations of) formulas as objects and denotations of proofs as morphisms.
	\end{lemma}
	...
	\begin{proof}
		...
		\begin{center}
			...
			\BinaryInfC{$B, A \vdash B \otimes A $}
			\RightLabel{ex.}
			\UnaryInfC{$A, B \vdash B \otimes A$}
			\RightLabel{$\otimes_L$}
			\UnaryInfC{$A \otimes B \vdash B \otimes A$}
...
%
		...
		Naturality and isomorphy of $\alpha$, $\rho$ and $\sigma$ as well as the coherence conditions are shown in the same way.
