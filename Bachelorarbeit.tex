...
\begin{document}
	...
	\begin{definition}[Formula]
		...
		\begin{itemize}
			...
			\item If $A$ and $B$ are formula, so are $A \otimes B$, $A \parr B$, $A \with B$ and $A \oplus B$.
		\end{itemize}
		%It is customary in linear logic to call $p^\bot$ atomic, for an atom $p$.
	\end{definition}

	Let $\Gamma $, $\Delta$ etc. be arbitrary, finite sequents of formulas (e.g.: $\Gamma = p_1, ..., p_n$) and $A$ and $B$ formulas.
	...
	\todo[inline]{Übergangstext}
	
	\begin{lemma}
		\label{key-cases}
		Let $\tau$ be a proof consisting of two subproofs $\tau_1$ and $\tau_2$ of strictly lower degree, whose last rules each introduce the principal formula $A$ respectively, i.e.: 
		\begin{center}
			$\tau = $ 
			\AxiomC{$\tau_1$} 
			\noLine
	...
	\begin{proof}
		...
		\begin{tabularx}{\textwidth}{x c x}
			...
			\scalebox{0.7}{
				...
				\BinaryInfC{$\Gamma_1, \Gamma_2, \Gamma_3 \vdash \Delta_1, \Delta_3, \Delta_2 $}
				\DisplayProof
			}
			...
			\scalebox{0.8}{
				...
				\noLine
				\UnaryInfC{$\svdots$}
				\UnaryInfC{$\oc\Gamma_1 \vdash \wn\Delta_1, A, \wn\Delta'_1$}
				\RightLabel{$\oc_R$}
				\UnaryInfC{$\oc\Gamma_1\vdash \wn\Delta_1, \oc A, \wn\Delta'_1 $}
				...
				\UnaryInfC{$\Gamma_2, A, \Gamma'_2 \vdash \Delta_2$}
				\RightLabel{$\oc_D$}
				\UnaryInfC{$\Gamma_2, \oc A, \Gamma'_2 \vdash \Delta_2$}
				
				\BinaryInfC{$\Gamma_2, \oc\Gamma_1, \Gamma'_2 \vdash \wn\Delta_1, \Delta_2, \wn\Delta'_1$}
				\DisplayProof
			}
			&
			$\rightsquigarrow$
	...
	\urgentnote[inline]{delete exponentials. They're obviously not part of the multiplicative fragment :| }
	...
	\todo[inline]{put that stuff back into a normal center instead of tabularx?}
	...
	and vice versa
	\urgentnote[inline]{think about the deltas...: aufteilen in zwei Regeln mit einseitigen Deltas für A und B oder allgemein halten und viele exchanges drunter klatschen?}
...
%---------------------------------------------------------------------------------------------------------------------
	...
	\todo[inline]{that's just changing the contexts $\Gamma_2$ and $\Gamma'_2$}
	...
	\begin{tabularx}{\textwidth}{x c x}
		...
		\noLine
		\UnaryInfC{$\svdots$}
		\UnaryInfC{$ \Gamma_2, A, B, \Gamma'_2 \vdash \Delta_2 $}
		\RightLabel{ex.L}
		\UnaryInfC{$ \Gamma_2, B, A, \Gamma'_2 \vdash \Delta_2 $}
		
		\BinaryInfC{$ \Gamma_2, B, \Gamma_1, \Gamma'_2 \vdash \Delta_1, \Delta_2, \Delta'_1 $}
		\DisplayProof
		&
		$\rightsquigarrow$
		&
		...
		\BinaryInfC{$ \Gamma_2, \Gamma_1, B, \Gamma'_2 \vdash \Delta_1, \Delta_2, \Delta'_1 $}
		
		\doubleLine
		\RightLabel{ex.L}
		\UnaryInfC{$ \Gamma_2, B, \Gamma_1, \Gamma'_2 \vdash \Delta_1, \Delta_2, \Delta'_1 $}
		\DisplayProof
	...
	\notimportantnote[inline]{gone to \ref{key-cases}}
	
	
	
	\subsubsection{Exponentials}
	\urgentnote[inline]{delete exponentials}
	
	\notimportantnote[inline]{partly gone to \ref{key-cases}}
