...
\begin{document}
	...
	\todo[inline]{move the LinLog stuff under the cat stuff?}
	...
	In order to regain our stable truths known from classical logic, we need to employ two unitary connectives $\oc$ ("of course" or "bang") and $\wn$ ("why not"). The bang operator informs us that there is an infinite amount of a resource: The statement $\oc A$ translates into the ownership of an amount of money that is large enough for us to ignore resource sensitivity. Imagine for example a billionaire buying a Pokemon figurine instead of a social media site: His amount of money will not be noticeably smaller after buying the figurine. We can informally say $\oc A = (1 \with A) \otimes (1 \with A) \otimes \cdots $ and therefore view classical and intuitionistic logic as some sort of limit of linear logic, just as classical mechanics is a limit of quantum mechanics and the theory of relativity. 
	...
	\end{definition}
	\todo[]{add more graph-like proof tree?}
...
%	Melliès to (multiplicative) classical linear logic and expanding a little bit on the proof given by Braüner \cite{brauner}.
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			\urgentnote[inline]{Problem: Resulting proofs for the $\parr_L$-cases are illegal under current formulation of the $\parr_L$-rule. (Also: they're kinda the same?)
				Possible solutions: \\
				1: add exchanges to transformation (easy; ugly)\\ 
				2: find generalized $\parr_L$-rule (more difficult; possibly uglier)}
				
			\todo[inline]{To be added: $\otimes_R$-cases.\\
			Maybe combine $\parr_L$- and $\otimes_R$-cases into one? (Requires generalized formulation of the rules)}
			...
			\todo[inline]{To be added: $\parr_L$- and $\otimes_R$-cases (same problems as above)}
...
\urgentnote[inline]{Weihnachtsbaustelle:}
	...
	\urgentnote[inline]{add definition of poly-categories?}
	...
	We will now define proof invariants as outlined by \urgentnote{add citation} Milliès.
	
	\begin{definition}[Proof invariant]
		Let $\pi$ be a proof tree. We call any function
		\begin{align*}
			\pi \mapsto [\pi]
		\end{align*}
		that remains constant under cut elimination \emph{proof invariant}. The entity $[\pi]$ is called \emph{denotation} of the proof $\pi$.
		\urgentnote{restrict definition on proofs of singular formula sequents}
	\end{definition}
	
	\todo[inline]{We're reachng categorical semantics. Move that stuff behind the category stuff.}
	
	\begin{definition}[Modularity]
		We call a proof invariant \emph{modular} iff there exists a binary operation $\circ$ such that for any two proofs
		\todo{is there a notation that's not as high as me on 4/20?}
		\AxiomC{$\pi_1 $}
		\noLine
		\UnaryInfC{$\svdots$}
		\UnaryInfC{$A \vdash B$} 
	...
	\begin{proposition}
		...
		\todo{more like polycategory if we allow general sequents...}
	\end{proposition}
	...
	\begin{lemma}
		A (co-)tensorial, modular proof invariant forms a monoidal category.
	\end{lemma}
	...
	\begin{proof}
		...
		\todo{requires generalized unit rules}
		\begin{center}
			\AxiomC{\strut}
			\RightLabel{$\id$}
			\UnaryInfC{$A \vdash A $}
...
%
		...
		\urgentnote[inline]{finish: naturality example on $\alpha$, isomorphy with $\id_{A \otimes B} \circ \rho^{-1} \circ \rho = \id_{A \otimes B} $}
	\end{proof}
	
	%______________________________________________________________Baustelle
	\begin{theorem}
	...
	\begin{proof}
		...
		\urgentnote[inline]{write $\partial$ proofs and maybe show coherence.}
	\end{proof}
	...
	\todo[inline]{is a separate definition even needed? integrate that stuff into expansions.}
	...
	\todo[]{put that stuff back into a normal center instead of tabularx?}
